...
class MLMRequest
{
	 ...
	 * @param	string	$type		Return type for the variable, for valid values see {@link MLMFilterInput::clean()}
	...
	function getVar($name, $default = null, $hash = 'default', $type = 'none', $mask = 0)
	{
		...
		if (isset($GLOBALS['_JREQUEST'][$name]['SET.'.$hash]) && ($GLOBALS['_JREQUEST'][$name]['SET.'.$hash] === true)) {
			...
			$var = MLMRequest::_cleanVar($var, $mask, $type);
		}
		elseif (!isset($GLOBALS['_JREQUEST'][$name][$sig]))
		{
			if (isset($input[$name]) && $input[$name] !== null) {
				// Get the variable from the input hash and clean it
				$var = MLMRequest::_cleanVar($input[$name], $mask, $type);

				// Handle magic quotes compatability
				if (get_magic_quotes_gpc() && ($var != $default) && ($hash != 'FILES')) {
					$var = MLMRequest::_stripSlashesRecursive( $var );
				}
			...
			elseif ($default !== null) {
				...
				$var = MLMRequest::_cleanVar($default, $mask, $type);
			}
			else {
				$var = $default;
			}
	...
	{
		return MLMRequest::getVar($name, $default, $hash, 'int');
	}
	...
	{
		return MLMRequest::getVar($name, $default, $hash, 'float');
	}
	...
	{
		return MLMRequest::getVar($name, $default, $hash, 'bool');
	}
	...
	{
		return MLMRequest::getVar($name, $default, $hash, 'word');
	}
	...
	{
		return MLMRequest::getVar($name, $default, $hash, 'cmd');
	}
	...
	function getString($name, $default = '', $hash = 'default', $mask = 0)
	{
		...
		return (string) MLMRequest::getVar($name, $default, $hash, 'string', $mask);
	}
	...
	function get($hash = 'default', $mask = 0)
	{
		...
		$result = MLMRequest::_cleanVar($input, $mask);

		// Handle magic quotes compatability
		if (get_magic_quotes_gpc() && ($hash != 'FILES')) {
			$result = MLMRequest::_stripSlashesRecursive( $result );
		}
	...
	function set( $array, $hash = 'default', $overwrite = true )
	{
		foreach ($array as $key => $value) {
			MLMRequest::setVar($key, $value, $hash, $overwrite);
		}
	}
	...
	function checkToken( $method = 'post' )
	{
		...
		if(!MLMRequest::getVar( $token, '', $method, 'alnum' )) {
...
;				$mainframe->redirect($return, JText::_('SESSION_EXPIRED'));
	...
	function clean()
	{
		MLMRequest::_cleanArray( $_FILES );
		MLMRequest::_cleanArray( $_ENV );
		MLMRequest::_cleanArray( $_GET );
		MLMRequest::_cleanArray( $_POST );
		MLMRequest::_cleanArray( $_COOKIE );
		MLMRequest::_cleanArray( $_SERVER );

		if (isset( $_SESSION )) {
			MLMRequest::_cleanArray( $_SESSION );
		}
	 ...
	 * @param string The variable type {@see MLMFilterInput::clean()}.
	 */
	function _cleanVar($var, $mask = 0, $type=null)
	{
		...
		elseif ($mask & 4)
		{
			...
			if (is_null($safeHtmlFilter)) {
				$safeHtmlFilter = & MLMFilterInput::getInstance(null, null, 1, 1);
			}
		...
		else
		{
			...
			if (is_null($noHtmlFilter)) {
				$noHtmlFilter = & MLMFilterInput::getInstance(/* $tags, $attr, $tag_method, $attr_method, $xss_auto */);
			}
	...
	{
		$value = is_array( $value ) ? array_map( array( 'MLMRequest', '_stripSlashesRecursive' ), $value ) : stripslashes( $value );
		return $value;
	}
}
