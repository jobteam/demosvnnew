...
/**
 * MLMFilterInput is a class for filtering input from any data source
 *
...
class MLMFilterInput
{
	 ...
	 * 		<pre>  $filter = & MLMFilterInput::getInstance();</pre>
	 *
	 * @static
	 * @param	array	$tagsArray	list of user-defined tags
	 * @param	array	$attrArray	list of user-defined attributes
	 ...
	 * @param	int		$attrMethod	WhiteList method = 0, BlackList method = 1
	 * @param	int		$xssAuto	Only auto clean essentials = 0, Allow clean blacklisted tags/attr = 1
	 * @return	object	The MLMFilterInput object.
	 * @since	1.5
	 */
	function & getInstance($tagsArray = array(), $attrArray = array(), $tagsMethod = 0, $attrMethod = 0, $xssAuto = 1)
	{
		...
		if (empty ($instances[$sig])) {
			$instances[$sig] = new MLMFilterInput($tagsArray, $attrArray, $tagsMethod, $attrMethod, $xssAuto);
		}
	...
	function clean($source, $type='string')
	{
		...
		switch (strtoupper($type))
		{
			...
			case 'STRING' :
				...
				if(isset($this) && is_a( $this, 'MLMFilterInput' )) {
					$filter =& $this;
				} else {
					$filter =& MLMFilterInput::getInstance();
				}
			...
			default :
				// Check for static usage and assign $filter the proper variable
				if(is_object($this) && get_class($this) == 'MLMFilterInput') {
					$filter =& $this;
				} else {
					$filter =& MLMFilterInput::getInstance();
				}
	...
	function _cleanAttributes($attrSet)
	{
		...
		for ($i = 0; $i < count($attrSet); $i ++)
		{
			...
			if (MLMFilterInput::checkAttribute($attrSubSet)) {
				continue;
			}
